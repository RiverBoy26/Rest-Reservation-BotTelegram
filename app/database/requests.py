...
async def set_table(table_id: int, number_of_seats: int, description: str):
    async with async_session() as session:
        table = Table(table_number=table_id, number_of_seats=number_of_seats,
...
async def delete_table(table_id: int):
    async with async_session() as session:
        table_to_delete = await session.scalar(select(Table).where(Table.table_number == table_id))
        await session.delete(table_to_delete)
        is_occupied_now_delete = await session.scalar(select(Tables_is_occupied_now).where(Tables_is_occupied_now.table_id == table_id))
        await session.delete(is_occupied_now_delete)
        await session.commit()


async def set_reservation(table_id: int, hour: int, minute: int,
                          surname: str, end_hour: int, end_minutes: int):
    async with async_session() as session:
        set_table_reservation = Table_reservation(table_id=table_id,
                                                  surname=surname,
                                                  hour=hour,
                                                  end_hour=end_hour,
                                                  minutes=minute,
...
async def get_reservation(table_id: int):
    async with async_session() as session:
        table = await session.scalars(select(Table_reservation)
                                      .where(Table_reservation.
                                             table_id == table_id)
                                      .order_by(Table_reservation.hour))

        reservation_info = table.all()

        if len(reservation_info) == 0:
            return "Отсутствует расписание"
        tbl_info = "".join([f"Забронирован на {'0'+str(t.hour) if t.hour < 10 else str(t.hour)}:{'0'+str(t.minutes) if t.minutes < 10 else str(t.minutes)}, "
                            f"{t.surname}, продолжительность - "
                            f"{t.end_hour}:{'0'+str(t.end_minutes) if t.end_minutes < 10 else str(t.end_minutes)}\n"
...
async def get_time_reservation(table_id: int):
    async with async_session() as session:
        ...
        for r in reservation_info:
            tbl_info.append(f"{'0'+str(r.hour) if r.hour < 10 else str(r.hour)}:{'0'+str(r.minutes) if r.minutes < 10 else str(r.minutes)}")
        return tbl_info

async def get_time_surname_and_chrono(table_id: int, hour: int, minute: int):
    async with async_session() as session:
        table = await session.scalar(select(Table_reservation)
                                      .where(Table_reservation
                                             .table_id == table_id,
                                             Table_reservation.hour == hour,
                                             Table_reservation.minutes == minute))

        return f"{table.surname}, Продолжительность: {table.end_hour}:{'0'+str(table.end_minutes) if table.end_minutes < 10 else str(table.end_minutes)}"
...
async def set_status_free(table_id: int):
    async with async_session() as session:
        status_update = (update(Tables_is_occupied_now)
                         .values(occupied_now=0)
                         .where(Tables_is_occupied_now.table_id == table_id))
